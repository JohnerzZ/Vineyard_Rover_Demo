...
class FeatureTracker
{
    private :
        ...
        // latest keyframe for reference pose of camera
        ...
        // predicted next pose of camera
        Eigen::Matrix4d predNPose = Eigen::Matrix4d::Identity();
        Eigen::Matrix4d predNPoseInv = Eigen::Matrix4d::Identity();
        // predicted reference pose of camera
        Eigen::Matrix4d predNPoseRef = Eigen::Matrix4d::Identity();

        // active keyframes size
        const int actvKFMaxSize {10};
        // minimum stereo matches
        const int minNStereo {80};
        // max added stereo matches if not enough
        const int maxAddedStereo {100};
        // minimum inliers for pose estimation
        const int minInliers {50};

        // percentage of matches to use for keyframe insertion
        float precCheckMatches {0.9f};
        // last keyframe number of mappoints tracked
        int lastKFTrackedNumb {0};

        // intrinsic parameters for back camera
        double fxb,fyb,cxb,cyb;

        // threshold number of frames passed from last keyframe
        const int keyFrameCountEnd {5};
        // number of frames passed from last keyframe
