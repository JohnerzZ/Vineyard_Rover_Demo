...
void ViewFrame::pangoQuit(Zed_Camera* zedPtr, const Map* _map)
{
    ...
    while( 1 )
    {
        // draw all needed items in the scene
...
// visualization of the map for Dual Camera Setup
...
void CameraFrame::drawPoints()
{
    ...
    for ( itw = mapMapP.begin(); itw != endw; itw ++)
    {
        ...
        // if mappoints are active draw them green, else white
...
// draw back camera if Dual Camera Setup
...
// draw lines from last keyframe to camera and lines from each keyframe to the next
void CameraFrame::lineFromKeyFrameToCamera()
{
    ...
    std::unordered_map<unsigned long, KeyFrame*>::const_iterator it, end(mapKeyF.end());
    glBegin(GL_LINES);
    // draw lines each keyframe to the next in red
    for ( it = mapKeyF.begin(); it != end; it ++)
    {
    ...
    glEnd();
    glColor3f(0.0f,1.0f,0.0f);
    // draw line from last keyframe to camera in green
