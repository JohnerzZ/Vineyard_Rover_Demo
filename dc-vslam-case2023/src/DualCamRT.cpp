...
std_msgs::Float64 eyaw;
ros::Publisher eyawPub;
...
int main (int argc, char **argv)
{
    ...
    eyawPub = 
        nh.advertise<std_msgs::Float64>("/estimated_yaw", 0);
...
void GetImagesROS::aprilTagCallBack(const DC_VSLAM::AprilTagDetectionArray::ConstPtr& msg)
{
    ...
    if ( !tagPoseFilled )
    {
        tagPose = voSLAM->mZedCamera->cameraPose.pose * T_c1_AT * Tc2_tag; //TODO 
        tagPoseFilled = true;
        return;
    }
...
{
    Eigen::Vector3d T;	       //get initial translation vector
    T << 1.7, 1.5, 0.05;       //1.7 instead of 1.5 because of 0.2 x offset due to camera offset from the center of the robot. Remember that
    Eigen::Matrix4d camPose = voSLAM->mZedCamera->cameraPose.pose;
    Eigen::Matrix3d rot = camPose.block<3,3>(0,0);
    Eigen::Matrix3d transfrot1; //transfrot1 is -pi/2 around the x-axis
    Eigen::Matrix3d transfrot2; //transfrot2 is -pi/2 around the z-axis
    transfrot1 << 1, 0, 0,
    		  0, 0, 1,
       	  0, -1, 0;
    transfrot2 << 0, 1, 0,
    ...
    tra = transfrot2 * transfrot1 * tra; //changing coordinate system (base link to camera default) for gazebo experiment
    tra = tra + T;
    rot = transfrot1*rot;       //I dont know why only transfrot1. 
    tf::Matrix3x3 tf_rot;
    matrixEigenToTF(rot, tf_rot);
    double roll, pitch, yaw;
    tf_rot.getRPY(roll, pitch, yaw);         //convert matrix to roll, pitch, yaw.
    roll = roll + 1.5709;
