...
// predict scale level of map point
...
// add connection to keyframe, keyframe, mappoint matched
void MapPoint::addConnection(KeyFrame* kF, const std::pair<int,int>& keyPos)
{
    // check if keyframe already matched
...
// add connection to keyframe, keyframe, mappoint matched for Dual Camera Setup
...
void MapPoint::update(KeyFrame* kF)
{
    ...
    int level {0};
    // update last observation and scale
    lastObsKF = kF;
    if ( idxs.second >= 0 )
    {
    ...
    // update maxDists for scale prediction
...
// update mappoint for back camera
...
// update position of the mappoint after local BA or loop closure
void MapPoint::updatePos(const Eigen::Vector3d& newPos, const Zed_Camera* zedPtr)
{
    // set world position
    setWordPose3d(newPos);
    // check if mappoint is close (accurate depth info)
...
// erase connection to KF (if wrong connection)
void MapPoint::eraseKFConnection(KeyFrame* kF)
{
    kFMatches.erase(kF);
}

// erase connection to KF (if wrong connection) for back camera
