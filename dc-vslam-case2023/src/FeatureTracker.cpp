...
// Feature Tracker constructor for Dual Camera Setup
...
{
    // extract left right image features with threads
...
{
    // extract left right image features with threads for Dual Camera Setup
    std::thread extractLeft(&FeatureExtractor::extractKeysNew, std::ref(feLeft), std::ref(leftIm), std::ref(keysLeft.keyPoints), std::ref(keysLeft.Desc));
    std::thread extractRight(&FeatureExtractor::extractKeysNew, std::ref(feRight), std::ref(rightIm), std::ref(keysLeft.rightKeyPoints),std::ref(keysLeft.rightDesc));
    extractLeft.join();
    extractRight.join();
...
void FeatureTracker::initializeMapR(TrackedKeys& keysLeft)
{
    ...
    {
        // add map points to keyframe if depth is available
        if ( keysLeft.estimatedDepth[i] > 0 )
        {
            // get 3d point from feature
    ...
    // set number of tracked keys for keyframe addition check
    ...
    // calculate inverse of last keyframe pose for pose prediction
    Eigen::Matrix4d lastKFPose = zedPtr->cameraPose.pose;
    lastKFPoseInv = lastKFPose.inverse();
}

// initialize map with first frame for Dual Camera Setup
void FeatureTracker::initializeMapRB(TrackedKeys& keysLeft, TrackedKeys& keysLeftB)
{
    ...
    int trckedKeys {0};
    // add map points to keyframe from front stereo camera
    for (size_t i{0}, end{keysLeft.keyPoints.size()}; i < end; i++)
    {
        // add map points to keyframe if depth is available
        if ( keysLeft.estimatedDepth[i] > 0 )
    ...
    // add map points to keyframe from back stereo camera
    ...
    map->addKeyFrame(kF);
    latestKF = kF;
    // add keyframe to keyframe vector
    allFrames.emplace_back(kF);
    // calculate inverse of last keyframe pose for pose prediction
    Eigen::Matrix4d lastKFPose = zedPtr->cameraPose.pose;
    lastKFPoseInv = lastKFPose.inverse();
...
// check reprojection error of map points for Dual Camera Setup
...
int FeatureTracker::findOutliersR(const Eigen::Matrix4d& estimPose, std::vector<MapPoint*>& activeMapPoints, TrackedKeys& keysLeft, std::vector<std::pair<int,int>>& matchesIdxs, const double thres, std::vector<bool>& MPsOutliers, const std::vector<float>& weights, int& nInliers)
{
    ...
    // get estimated pose of right camera
    ...
    for (size_t i {0}, end{matchesIdxs.size()}; i < end; i++)
    {
        ...
        // if no mappoint continue
        if ( !mp )
            continue;
        // calculate projection of mappoint (left, right)
        ...
        // calculate weight depending on octave of feature (pyramid level)
        ...
        {
            nInliers++;
            // check if mappoint is close enough to be considered a stereo match
...
// find outliers in map points for Dual Camera Setup
...
std::pair<int,int> FeatureTracker::estimatePoseCeresR(std::vector<MapPoint*>& activeMapPoints, TrackedKeys& keysLeft, std::vector<std::pair<int,int>>& matchesIdxs, Eigen::Matrix4d& estimPose, std::vector<bool>& MPsOutliers, const bool first)
{
    ...
    for (size_t iter {0}; iter < maxIter; iter ++ )
    {
        ...
        for (size_t i{0}, end{matchesIdxs.size()}; i < end; i++)
        {
            ...
            // check if left or right feature is matched
            if ( keyPos.first >= 0 )
            {
                if  ( !mp->inFrame )
                    continue;
                const int nIdx {keyPos.first};
                // check if feature is close enough to be considered a stereo match
                if (  keysLeft.close[nIdx] )
                {
                    // if close enough, consider it twice, one left, one right, for scale information
                    // get observation
                    ...
                    // get weight depending on octave of feature (pyramid level)
                ...
                {
                    // if right feature is not close enough, only add left feature
                    Eigen::Vector2d obs((double)keysLeft.keyPoints[nIdx].pt.x, (double)keysLeft.keyPoints[nIdx].pt.y);
                    Eigen::Vector3d point = mp->getWordPose3d();
                    const int octL = keysLeft.keyPoints[nIdx].octave;
                    const double weight = (double)feLeft->InvSigmaFactor[octL];
            ...
            // if right feature is matched only, add it
            else if ( keyPos.second >= 0)
            {
                if  ( !mp->inFrameR )
                    continue;
...
// estimate pose using ceres for Dual Camera Setup
std::pair<std::pair<int,int>, std::pair<int,int>> FeatureTracker::estimatePoseCeresRB(std::vector<MapPoint*>& activeMapPoints, TrackedKeys& keysLeft, std::vector<std::pair<int,int>>& matchesIdxs, std::vector<bool>& MPsOutliers, std::vector<MapPoint*>& activeMapPointsB, TrackedKeys& keysLeftB, std::vector<std::pair<int,int>>& matchesIdxsB, std::vector<bool>& MPsOutliersB, Eigen::Matrix4d& estimPose)
{
    ...
    // get all needed transformations
    const Eigen::Matrix4d estimPoseRInv = zedPtr->extrinsics.inverse();
    const Eigen::Matrix3d qc1c2 = estimPoseRInv.block<3,3>(0,0);
    const Eigen::Matrix<double,3,1> tc1c2 = estimPoseRInv.block<3,1>(0,3);
    ...
    for (size_t iter {0}; iter < maxIter; iter ++ )
    {
        ...
        Eigen::Matrix4d frame_pose = estimPose;
        Eigen::Matrix3d frame_R;
        // initial estimated pose
        frame_R = frame_pose.block<3, 3>(0, 0);
        frame_tcw = frame_pose.block<3, 1>(0, 3);
        frame_qcw = Eigen::Quaterniond(frame_R);
        // set quaternion local parameterization
        ceres::Manifold* quaternion_local_parameterization = new ceres::EigenQuaternionManifold;
        // set Huber loss function
        ceres::LossFunction* loss_function = new ceres::HuberLoss(sqrt(7.815f));
        // loop through all matches of front camera
        for (size_t i{0}, end{matchesIdxs.size()}; i < end; i++)
        {
            if ( MPsOutliers[i] )
                continue;
            ...
            if ( mp->GetIsOutlier() )
                continue;
            ceres::CostFunction* costf;
            // check if left or right keypoint
            if ( keyPos.first >= 0 )
            {
                if  ( !mp->inFrame )
                    continue;
        ...
        // the same procedure for the back camera
...
// predict position of mappoints in the next frame
bool FeatureTracker::worldToFrameRTrack(MapPoint* mp, const bool right, const Eigen::Matrix4d& predPoseInv, const Eigen::Matrix4d& tempPose)
{
    ...
    // transform to predicted camera frame
    ...
    // check if map point is in front of the camera
    if ( point(2) <= 0.0 )
    {
        if ( right )
            mp->inFrameR = false;
    ...
    // predict scale of the map point
    int predScale = mp->predictScale(dist);

    // assign prediction to the map point
    if ( right )
    {
...
// predict position of mappoints in the next frame for Dual Camera Setup
...
{
    // calculate reference pose of the new keyframe
    ...
    int trckedKeys {0};
    // loop through all matched mappoints
    for ( size_t i{0}, end {matchesIdxs.size()}; i < end; i++)
    {
        MapPoint* mp = activeMapPoints[i];
        std::pair<int,int>& keyPos = matchesIdxs[i];
        ...
        if ( MPsOutliers[i] )
            continue;
        // add the keyframe to the map point
        mp->kFMatches.insert(std::pair<KeyFrame*, std::pair<int,int>>(kF, keyPos));
        // update the map point (calculate the descriptor)
        mp->update(kF);
    ...
    // if there are not enough stereo matches, add the 100 closest stereo matches
    if ( nStereo < minNStereo)
    {
        ...
        // sort the depths of the stereo matches
        std::sort(allDepths.begin(), allDepths.end());
        int count {0};
        // add 100 closest stereo matches (if they are not matched already)
    ...
    // calculate all connections between the new keyframe and the previous ones (for local mapping)
    kF->calcConnections();
    lastKFTrackedNumb = trckedKeys;
    ...
    lastKFPoseInv = lastKFPose.inverse();
    allFrames.emplace_back(kF);
    // if apriltag is detected, start loop closure or register the apriltag pose
    if ( map->aprilTagDetected && !map->LCStart )
    {
        map->LCStart = true;
        map->LCCandIdx = kF->numb;
    }
    // if there are more than 3 keyframes, start local BA
    if ( map->keyFrames.size() > 3 && !map->LCStart )
        map->keyFrameAdded = true;

}

// insert a new keyframe for Dual Camera Setup
void FeatureTracker::insertKeyFrameRB(TrackedKeys& keysLeft, std::vector<int>& matchedIdxsL, std::vector<std::pair<int,int>>& matchesIdxs, std::vector<bool>& MPsOutliers, TrackedKeys& keysLeftB, std::vector<int>& matchedIdxsLB, std::vector<std::pair<int,int>>& matchesIdxsB, std::vector<bool>& MPsOutliersB, const int nStereo, const int nStereoB, const Eigen::Matrix4d& estimPose, cv::Mat& leftIm, cv::Mat& rleftIm)
{
    ...
    std::lock_guard<std::mutex> lock(map->mapMutex);
    int trckedKeys {0};
    // loop through all matched mappoints of front camera
    for ( size_t i{0}, end {matchesIdxs.size()}; i < end; i++)
    {
        MapPoint* mp = activeMapPoints[i];
        std::pair<int,int>& keyPos = matchesIdxs[i];
        ...
        if ( MPsOutliers[i] )
            continue;
        // add keyframe to the mappoint
        mp->kFMatches.insert(std::pair<KeyFrame*, std::pair<int,int>>(kF, keyPos));
        // update the mappoint (calculate the descriptor)
        mp->update(kF);
    ...
    // loop through all matched mappoints of back camera
    for ( size_t i{0}, end {matchesIdxsB.size()}; i < end; i++)
    {
        MapPoint* mp = activeMapPointsB[i];
    ...
    // if not enough stereo matches on the front camera add the 100 closest
    if ( nStereo < minNStereo)
    {
    ...
    // if not enough stereo matches on the back camera add the 100 closest
...
// add the frame to the map if it is not a keyframe (for complete trajectory)
...
// change the poses of the keyframes after loop closure
void FeatureTracker::changePosesLCA(const int endIdx)
{
    // get the last keyframe that was optimized
    KeyFrame* kf = map->keyFrames.at(endIdx);
    // update the poses of all the next keyframes
    while ( true )
    {
    ...
    // get the predicted pose of the camera in the next frame
    predNPose = zedPtr->cameraPose.pose * predNPoseRef;
    predNPoseInv = predNPose.inverse();

}

// change the poses of the keyframes after loop closure for Dual Camera Setup
...
// remove the map points that are not in the frame for Dual Camera Setup
void FeatureTracker::removeOutOfFrameMPsRB(const Zed_Camera* zedCam, const Eigen::Matrix4d& predNPose, std::vector<MapPoint*>& activeMapPoints)
{
    ...
    // calculate the transformation from the camera to the world frame for the left and right lens
    Eigen::Matrix4d toRCamera = (predNPose * zedCam->extrinsics).inverse();
    Eigen::Matrix4d toCamera = predNPose.inverse();
    int j {0};
    // check all active mappoints if they are in the frame (either left or right lens)
    ...
    // remove the inactive mappoints
    activeMapPoints.resize(j);
}

// remove the map points that are not in the frame
...
// predict the position of the active mappoints in the next frame
void FeatureTracker::newPredictMPs(const Eigen::Matrix4d& currCamPose, const Eigen::Matrix4d& predNPose, std::vector<MapPoint*>& activeMapPoints, std::vector<int>& matchedIdxsL, std::vector<int>& matchedIdxsR, std::vector<std::pair<int,int>>& matchesIdxs, std::vector<bool> &MPsOutliers)
{
    ...
    Eigen::Matrix4d tempR = currCamPose.inverse() * (predNPose * zedPtr->extrinsics);
    std::lock_guard<std::mutex> lock(map->mapMutex);
    // predict the position of the mappoints and check if they are in frame or if they are in front of the camera
    for ( size_t i {0}; i < end; i++)
    {
        MapPoint* mp = activeMapPoints[i];
        if ( !mp )
...
// predict the position of the active mappoints in the next frame for Dual Camera Setup
...
// set mappoints to outliers if they are not matched for 20+ frames or if they are found to be outliers
...
// the main function (tracks the features in the image estimates the pose of the camera)
void FeatureTracker::TrackImageT(const cv::Mat& leftRect, const cv::Mat& rightRect, const int frameNumb)
{
    curFrame = frameNumb;
    curFrameNumb++;
    
    // if local BA or loop closure is done, update the poses of the mappoints
    ...
    // if first frame initialize the map
    if ( curFrameNumb == 0 )
    {
    ...
    // remove mappoints from previous frame and predict their position
    ...
    // find features and stereo match them
    extractORBStereoMatchR(leftIm, rightIm, keysLeft);
    ...
    float rad {10.0};
    // if second frame, there is no camera pose prediction, so use a larger radius for matching
    if ( curFrameNumb == 1 )
        rad = 120;
    else
        rad = 10;
    ...
    bool toBreak {false};
    int countIte {0};
    // if not enough inliers are found, increase the radius and try again
    while ( nIn.first < minInliers )
    {
        ...
        // reset if not enough inliers are found
    ...
    // publish the pose and calculate the next predicted pose
    publishPoseNew();

    setActiveOutliers(activeMpsTemp,MPsOutliers, matchesIdxs);
}

// the main function (tracks the features in the image estimates the pose of the camera) for Dual Camera Setup
void FeatureTracker::TrackImageTB(const cv::Mat& leftRect, const cv::Mat& rightRect, const cv::Mat& leftRectB, const cv::Mat& rightRectB, const int frameNumb)
{
    ...
    // if first frame extract the features from both front and back camera images and stereo match them with threads and initialize the map
    if ( curFrameNumb == 0 )
    {
...
// function to draw the tracked keypoints
...
// function to publish the estimated pose and predict the next one
...
// function to publish the estimated pose and predict the next one for Dual Camera Setup
