    ...
    for ( int row {minY}; row <= maxY; row ++)
    {
        for ( int col {minX}; col <= maxX; col++)
        {
            ...
            for (size_t i {0}, end{grid.size()}; i < end; i++)
            {
                ...
                // add the index if the distance is within the radius
...
// match the features between the last keyframe and the new keyframe for local bundle adjustment
    ...
    for ( size_t i {0}, end{p4d.size()}; i < end; i++)
    {
        ...
        if ( pLeft.x > 0 && pLeft.y > 0 )
        {
            ...
            if ( !idxs.empty() )
            {
                for (auto& idx : idxs)
                {
                    // check if the feature is already matched
        ...
        int bestLevR2 = -1;
        int secDistR = 256;
        // get the best match for the right feature
...
// match the features between the last keyframe and the new keyframe for local bundle adjustment for both front and back camera
...
{
    int nMatches {0};
    // get correct variables depending on the camera
    ...
    for ( size_t i {0}, end{p4d.size()}; i < end; i++)
    {
        ...
        int secDist = 256;
        const cv::Point2f& pLeft = predPoints[i].first;
        // get the radius depending on the scale of the feature
        float radius = feLeft->scalePyramid[predScale] * rad;
        if ( pLeft.x > 0 && pLeft.y > 0 )
        {
            cv::Point2f prevKeyPos;
...
// match by projection between active map points and new features
int FeatureMatcher::matchByProjectionRPred(std::vector<MapPoint*>& activeMapPoints, TrackedKeys& keysLeft, std::vector<int>& matchedIdxsL, std::vector<int>& matchedIdxsR, std::vector<std::pair<int,int>>& matchesIdxs, const float rad)
{
    ...
    // loop through the active map points
    for ( size_t i {0}, prevE{activeMapPoints.size()}; i < prevE; i++)
    {
        ...
        std::vector<int> idxs;
        // get the indexes of the features in the radius
...
// stereo matching left right images
void FeatureMatcher::findStereoMatchesORB2R(const cv::Mat& lImage, const cv::Mat& rImage, const cv::Mat& rightDesc,  std::vector<cv::KeyPoint>& rightKeys, TrackedKeys& keysLeft)
{
    ...
    // distribute the right keys according to their y position
    ...
    // loop through all left features
    for (it = keysLeft.keyPoints.begin(); it != end; it++, leftRow++)
    {
        ...
        {
            // find the best match in the right image according to the y position
        ...
        // loop through all possible x positions of the window and find best distance
    ...
    // remove 1% of the closest points
    ...
    // remove matches with distance greater than 1.5*1.4 times the median
...
// calcuate distance between two descriptors
...
// distribute left keys into rows according to their y coordinate
