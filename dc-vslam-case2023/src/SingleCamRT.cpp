...
// publish ground truth pose of the robot
#define GTPOSE false
#define GTPOSEDIF false
// publish pointcloud (for gazebo)
#define PUBPOINTCLOUD false
// move robot with move base package to optimal pose for apriltag detection
#define ATBESTPOSE false

// for real time ROS is used
class GetImagesROS
{
    public:
        ...
        // save ground truth trajectory and positions
        ...
        // current goal subscriber for optimal pose
        ...
        // vector with ground truth positions
        std::vector<Eigen::Vector3d> gtPositions;
        // vector with ground truth quaternions
        std::vector<Eigen::Quaterniond> gtQuaternions;

        // transformation world to camera 1
        Eigen::Matrix4d T_w_c1;
        // transformation camera 1 to apriltag
        Eigen::Matrix4d T_c1_AT = Eigen::Matrix4d::Identity();
        // transformation world to apriltag
        Eigen::Matrix4d tagPose;
        // transformation world to apriltag from config file
        Eigen::Matrix4d tagPoseW;
        // transformation apriltag to camera 1 optimal
        Eigen::Matrix4d T_tag_b;
        // transformation base frame to camera 1
        Eigen::Matrix4d T_bf_c1;
        // transformation optimal pose
        ...
        // rect map for rectification
        cv::Mat rectMap[2][2];
        // frame number
        int frameNumb {0};
        // times apriltag was found 
        int ATFoundCount {0};
        // times apriltag was lost 
        int ATLostCount {0};
        // if apriltag was found
        bool ATFound {false};
        // if tag pose has been registered
        bool tagPoseFilled {false};
        // if robot on track 
        bool onTrack {false};
        bool first {true};
        bool pathSuccess {false};
        // if previous goal is registered
...
int main (int argc, char **argv)
{
    ...
    // pre open opencv windows for images
    ...
    // if ground truth pose is available
    bool gtPose {false};
    try
    {
    ...
    // if same camera is used for SLAM and apriltag detection
    bool LCSameCam {true};
    try
    {
    ...
    // itilialize subscribers for images
...
BESTPOSE
    ...
    // if ground truth with AT for csl
#elif GTPOSEDIF
...
// get the pose from apriltag detection
...
// if optimal pose when apriltag is detected robot moves to optimal pose for apriltag detection
#if ATBESTPOSE
...
void GetImagesROS::aprilTagCallBack(const DC_VSLAM::AprilTagDetectionArray::ConstPtr& msg)
{
    ...
    // when apriltag is detected get the pose
    ...
    // get apriltag pose in world frame if not registered
    ...
    // if apriltag registered, calculate camera pose and initiate loop closure
...
// apriltag callback without optimal pose
void GetImagesROS::aprilTagCallBack(const DC_VSLAM::AprilTagDetectionArray::ConstPtr& msg)
{
    if ( voSLAM->map->aprilTagDetected )
        return;
    // if apriltag not found for 10 continous frames start detection again
    if ( msg->detections.size() == 0 )
    {
        if ( ATFound )
            ATLostCount++;
    ...
    Tc2_tag.block<3,3>(0,0) = qTag.toRotationMatrix();
    Tc2_tag.block<3,1>(0,3) = tTag;
    // if apriltag not registered, get pose in world frame
    if ( !tagPoseFilled )
    {
...
// save ground truth trajectory and positions with transformations needed
...
void GetImagesROS::getImages(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight,const nav_msgs::OdometryConstPtr& msgGT)
{
    ...
    for (size_t i {0}, end{actMPs.size()}; i < end; i ++)
    {
        ...
        // check if the map point is valid and its position to fit the height of robot
        // obstacle avoidance only for mappoints up to the same height of the robot
...
// publish odom with needed transformation 
