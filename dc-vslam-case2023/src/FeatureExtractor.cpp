...
// bit pattern used by the BRIEF descriptor
...
// compute the descriptor (from OpenCV ORB)
...
// compute the descriptors (from OpenCV ORB)
...
float FeatureExtractor::computeOrientation(const cv::Mat& image, const cv::Point2f& point)
{
    ...
    // return the patch orientation
    return cv::fastAtan2((float)m_01, (float)m_10);

}

// precompute image pyramid
...
// ssc function from https://github.com/BAILOOL/ANMS-Codes
...
void FeatureExtractor::computeKeypointsORBNew(cv::Mat& image, std::vector<std::vector<cv::KeyPoint>>& allKeys)
{
    ...
    // separate images in grids

    // fast needs 3 pixels of border to find corners
    ...
    for (size_t level {0}; level < nLevels; level ++)
    {
        ...
        // find fast features in each grid
        for (int iR = 0; iR < nRows; iR++)
        {
            ...
            for (int iC = 0; iC < nCols; iC++)
            {
                ...
                if (!temp.empty())
                {
                    // add border to feature coordinates
        ...
        // keep the best features for each level with ssc
...
FeatureExtractor::FeatureExtractor(const int _nfeatures, const int _nLevels, const float _imScale, const int _edgeThreshold, const int _patchSize, const int _maxFastThreshold, const int _minFastThreshold) : nFeatures(_nfeatures), nLevels(_nLevels), imScale(_imScale), edgeThreshold(_edgeThreshold), patchSize(_patchSize), maxFastThreshold(_maxFastThreshold), minFastThreshold(_minFastThreshold)
{
    ...
    // precalculate all needed parameters (scale for each pyramid level, features per level, etc.)
    ...
    // calculate feature per level (from OpenCV)
    ...
    // precompute the pattern for the orientation (for the length of each row on the circle)
